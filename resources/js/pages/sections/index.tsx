...
export default function SectionsIndex() {
  ...
  function toggle(section: Section) {
    router.put(sectionUpdate.url(section.id), {
      is_unlocked: !section.pivot?.is_unlocked,
    })
  }
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <span className="text-sm font-normal text-muted-foreground">
                {sections.filter(s => s.pivot?.is_unlocked).length}
                {' '}
                /
                {sections.length}
                {' '}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      ...
                      className={section.pivot?.is_unlocked ? 'text-foreground hover:underline' : 'text-muted-foreground hover:underline'}
                    >
                      {section.title}
                    </Link>
                  ...
                  <button
                    ...
                    title={section.pivot?.is_unlocked ? 'Lock section' : 'Unlock section'}
                  >
                    {section.pivot?.is_unlocked
